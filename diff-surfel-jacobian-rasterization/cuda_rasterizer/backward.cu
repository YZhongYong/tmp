...
renderCUDA(
	...
	float* __restrict__ abs_dL_dmean2D)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (rho3d <= rho2d) {
				...
				//! update abs dL_dmean2D
				atomicAdd(&abs_dL_dmean2D[global_id * 2 + 0], abs(dL_dTu.z));
				atomicAdd(&abs_dL_dmean2D[global_id * 2 + 1], abs(dL_dTv.z));
				

				// atomicAdd(&abs_dL_dmean2D[global_id * 2 + 0], dL_dTu.z * dL_dTu.z);
				// atomicAdd(&abs_dL_dmean2D[global_id * 2 + 1], dL_dTv.z * dL_dTv.z);
...
// //? TW added
// __device__ void normalizeBackward(float3 &dL_dtn, const float3 &normal, const float length) {
//     if (length > FLT_EPSILON) {
//         // dL_dnorm은 정규화된 벡터에 대한 그레디언트
//         float3 dL_dnorm = make_float3(dL_dtn.x / length, dL_dtn.y / length, dL_dtn.z / length);
//         float dot_product = dL_dtn.x * normal.x + dL_dtn.y * normal.y + dL_dtn.z * normal.z;
//         float inv_length_squared = 1.0f / (length * length);

//         dL_dtn.x = (dL_dnorm.x - normal.x * dot_product * inv_length_squared) / length;
//         dL_dtn.y = (dL_dnorm.y - normal.y * dot_product * inv_length_squared) / length;
//         dL_dtn.z = (dL_dnorm.z - normal.z * dot_product * inv_length_squared) / length;
//     } else {
//         dL_dtn.x = 0.0f;
//         dL_dtn.y = 0.0f;
//         dL_dtn.z = 0.0f;
...
__device__ void normalizeBackward(float3 &dL_dtn, const float3 &normal, const float length) {
    if (length > FLT_EPSILON) {
        ...
        float inv_length_cubed = inv_length * inv_length * inv_length;

        // dv^/dv 행렬의 각 성분을 계산합니다.
        float3 row0, row1, row2;

        // 첫 번째 행
        // row0.x = inv_length - (normal.x * normal.x) * inv_length_cubed;//(sqrt(x^2+y^2+z^2)^2-x^2)
		row0.x = ((normal.y * normal.y) + (normal.z * normal.z))  * inv_length_cubed;//(y^2+z^2)
        row0.y = -(normal.x * normal.y) * inv_length_cubed;
        row0.z = -(normal.x * normal.z) * inv_length_cubed;

        // 두 번째 행
        row1.x = -(normal.y * normal.x) * inv_length_cubed;
        // row1.y = inv_length - (normal.y * normal.y) * inv_length_cubed;
		row1.y = ((normal.x * normal.x) + (normal.z * normal.z))  * inv_length_cubed;
        row1.z = -(normal.y * normal.z) * inv_length_cubed;

        // 세 번째 행
        row2.x = -(normal.z * normal.x) * inv_length_cubed;
        row2.y = -(normal.z * normal.y) * inv_length_cubed;
        // row2.z = inv_length - (normal.z * normal.z) * inv_length_cubed;
		row2.z = ((normal.y * normal.y) + (normal.x * normal.x))  * inv_length_cubed;

        // dL/dv 계산
        float3 result;
        result.x = row0.x * dL_dtn.x + row0.y * dL_dtn.y + row0.z * dL_dtn.z;
        result.y = row1.x * dL_dtn.x + row1.y * dL_dtn.y + row1.z * dL_dtn.z;
        result.z = row2.x * dL_dtn.x + row2.y * dL_dtn.y + row2.z * dL_dtn.z;
        dL_dtn = result;
    } else {
		// printf("123???");
        dL_dtn.x = 0.0f;
	 ...
	 glm::vec4* dL_drots)
{
	...
	glm::mat3x4 dL_dM = P * glm::transpose(dL_dT); //! T = M.T * P //! dL_dM = dL_dT * P.transpose
	...
	// float normal_length;
	// normalize(normal, normal_length); // Ensure /normal is normalized

	// float length = (normal.x * normal.x + normal.y * normal.y + normal.z * normal.z);
	float length = sqrt(normal.x * normal.x + normal.y * normal.y + normal.z * normal.z);
	// normalizeBackward(dL_dtn, normal, length);


	// normalizeBackward(dL_dtn, normal, normal_length);

	glm::mat3 J_T = glm::transpose(J);//*
	glm::mat3 dL_dJRS = glm::mat3(
		glm::vec3(dL_dM[0]),
		glm::vec3(dL_dM[1]),
		glm::vec3(dL_dtn.x, dL_dtn.y, dL_dtn.z)
	);

	//? add normal, length and normalizeBackward(dL_dnormal, normal, length);
	glm::mat3 J_inv_T = glm::transpose(J_inv);//*
	glm::mat3 dL_dR =  glm::mat3(
		(J_T * dL_dJRS[0]) * glm::vec3(scale.x),
		(J_T * dL_dJRS[1]) * glm::vec3(scale.y),
		(J_inv_T * dL_dJRS[2]));//*
	// glm::mat3 dL_dR =  glm::mat3(
	// 	(dL_dJRS[0]) * glm::vec3(scale.x),
	// 	(dL_dJRS[1]) * glm::vec3(scale.y),
	// 	dL_dJRS[2]);
	...
	dL_dscales[idx] = glm::vec2(
		(float)glm::dot(J * R[0], dL_dJRS[0]), // x축에 대한 스케일
		(float)glm::dot(J * R[1], dL_dJRS[1])  // y축에 대한 스케일
	);//*
	// dL_dscales[idx] = glm::vec2(
	// 	(float)glm::dot(R[0], dL_dJRS[0]), // x축에 대한 스케일
	// 	(float)glm::dot(R[1], dL_dJRS[1])  // y축에 대한 스케일
	// );
	...
	float* abs_dL_dmean2D)
{
	...
	compute_transmat_aabb(
		idx, 
		Ts_precomp,
		means3D, scales, rotations, Jacobians, Jacobians_inv,
	...
	//!
	// dL_dmean2Ds[idx].z = abs_dL_dmean2D[idx * 2 + 0] * depth * 0.5 * float(W) + abs_dL_dmean2D[idx * 2 + 1] * depth * 0.5 * float(H); // ! l1
	// dL_dmean2Ds[idx].z = sqrt(abs_dL_dmean2D[idx * 2 + 0] + abs_dL_dmean2D[idx * 2 + 1])  * depth * 0.5 * float(H); // ! l2
